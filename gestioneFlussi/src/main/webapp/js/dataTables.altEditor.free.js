 ...
 * @author kingkode (www.kingkode.com) Modified by: Kasper Olesen
 *         (https://github.com/KasperOlesen), Luca Vercelli
 *         (https://github.com/luca-vercelli), Zack Hable
 *         (www.cobaltdevteam.com)
	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($, window, document, undefined) {
			...
			var altEditor = function(dt, opts) {
				...
				/**
				 * @namespace Settings object which contains customisable
				 *            information for altEditor instance
				 */
				this.s = {
					...
					/**
					 * @type {String} Unique namespace for events attached to
					 *       the document
					 */
				...
				/**
				 * @namespace Common and useful DOM elements for the class
				 *            instance
				 */
				this.dom = {
							...
							altEditor.prototype,
							{
								...
								_setup : function() {
									...
									if (dt.button('delete:name')) {
										dt.button('delete:name').action(
												function(e, dt, node, config) {
													that._openDeleteModal();
												});

										$(this.modal_selector).on(
												'click',
												'#deleteRowBtn',
												function(e) {
													e.preventDefault();
								...
								_openEditModal : function() {
									...
									for ( var j in columnDefs) {
										...
										if (columnDefs[j].type
											...
											// handle fields that are visible to
											// the user
											...
											// Adding text-inputs and
											// errorlabels
											if (columnDefs[j].type
													.indexOf("text") >= 0) {
												data += "<input type='"
														+ that
																._quoteattr(columnDefs[j].type)
														+ "' id='"
														+ that
																._quoteattr(columnDefs[j].name)
														+ "'  pattern='"
														+ that
																._quoteattr(columnDefs[j].pattern)
														+ "'  title='"
														+ that
																._quoteattr(columnDefs[j].hoverMsg)
														+ "' name='"
														+ that
																._quoteattr(columnDefs[j].title)
														+ "' placeholder='"
														+ that
																._quoteattr(columnDefs[j].title)
														+ "' data-special='"
														+ that
																._quoteattr(columnDefs[j].special)
														+ "' data-errorMsg='"
														+ that
																._quoteattr(columnDefs[j].msg)
														+ "' data-uniqueMsg='"
														+ that
																._quoteattr(columnDefs[j].uniqueMsg)
														+ "' data-unique='"
														+ columnDefs[j].unique
														+ "'"
														+ (columnDefs[j].maxLength == false ? ""
																: " maxlength='"
								...
								_editRowData : function() {
									...
									that.onEditRow(that, rowDataArray,
											function(data, b, c, d, e) {
												that._editRowCallback(data, b,
														c, d, e);
											}, function(data) {
									...
									for ( var i in dt.context[0].aoColumns) {
										columnDefs
												.push({
													...
													type : (dt.context[0].aoColumns[i].type ? dt.context[0].aoColumns[i].type
															: 'text'),
													name : dt.context[0].aoColumns[i].data ? dt.context[0].aoColumns[i].data
								...
								_deleteRow : function() {
									...
									// Getting the IDs and Values of the
									// tablerow
									for (var i = 0; i < dt.context[0].aoColumns.length; i++) {
										// .data is the attribute name, if any;
										// .idx is the column
										...
										if (columnDefs[j].type
											...
											if (columnDefs[j].type
												...
												for (var i = 0; i < columnDefs[j].options.length; i++) {
													options += "<option value='"
															+ that
																	._quoteattr(columnDefs[j].options[i])
															+ "'>"
															+ columnDefs[j].options[i]
															+ "</option>";
												}
								...
								_addRowData : function() {
									...
									that.onAddRow(that, rowDataArray, function(
											data) {
								 ...
								 */
								_deleteRowCallback : function(response, status,
										more) {
									var selector = this.modal_selector;
									...
									$("div" + selector)
											.find("button#addRowBtn").prop(
													'disabled', true);
									$("div" + selector).find(
											"button#editRowBtn").prop(
											'disabled', true);
									$("div" + selector).find(
											"button#deleteRowBtn").prop(
											'disabled', true);
								},

								/**
								 * Called after a row has been inserted on
								 * server
								 */
								_addRowCallback : function(response, status,
										more) {
								 ...
								 */
								_editRowCallback : function(response, status,
										more) {
								 ...
								 */
								_errorCallback : function(response, status,
										more) {
									var error = response;
									var selector = this.modal_selector;
									$(selector + ' .modal-body .alert')
								...
								/**
								 * Default callback for insertion: mock
								 * webservice, always success.
								 */
								onAddRow : function(dt, rowdata, success, error) {
									console
											.log("Missing AJAX configuration for INSERT");
									success(rowdata);
								},

								/**
								 * Default callback for editing: mock
								 * webservice, always success.
								 */
								onEditRow : function(dt, rowdata, success,
										error) {
									console
											.log("Missing AJAX configuration for UPDATE");
									success(rowdata);
								},

								/**
								 * Default callback for deletion: mock
								 * webservice, always success.
								 */
								onDeleteRow : function(dt, rowdata, success,
										error) {
									console
											.log("Missing AJAX configuration for DELETE");
									success(rowdata);
								},
			...
			altEditor.defaults = {
				/**
				 * @type {Boolean} Ask user what they want to do, even for a
				 *       single option
				 */
				alwaysAsk : false,
				...
				/**
				 * @type {DataTable.Editor} Editor instance for automatic
				 *       submission
				 */
				editor : null
			};
